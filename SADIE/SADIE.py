...
try:
	...
	#count how many previous shifts were ON 1,2,3 and IN 1,2,3
	prev_shifts_on = {}
	prev_shifts_in = {}
	for name in names:
		prev_shifts_on[name] = {}
	...
	#make dictionary of what was most recents shift before current scheduling period
	hist_dict = {}
	for name in names:
	...
	def run_LP(days_off_on = 1, days_off_in = 1, days_off_ei = 1, max_days_week = 3, on_tol = (0,1), in_tol = (0,1), tot_tol = (0,1), on_reward = 2, in_reward = 1):
		...
		#require that number of shifts per person is bounded below and above
		for j in range(n):
		...
		#ensure correct number of shifts scheduled each day
		for j in range(m):
		...
		#set minimum time off for each worker
		for i in range(n):
			l = on_vars[i]
			p = in_vars[i]
			for j in range(m-6):
		...
		#ensure no worker schedule IN and ON on a given day
		for i in range(n):
			for j in range(m):
		...
		#add OFF and IN PREF constraints
		run_sum = 0
		for i in range(n):
			for j in range(m):
				pref = total_df.ix[:,j][key_names[i]]
	...
	#sort out which ON duties and which IN duties workers get on assigned days
